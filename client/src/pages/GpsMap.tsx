...
import { MapPin, Navigation, Satellite, Map, Mountain, Route, Clock } from "lucide-react";
...
// Distinct colors for tracks — up to 8 ISsIs before cycling
const TRACK_COLORS = [
...
export default function GpsMap() {
  ...
  // Assign a stable color index per ISSI
      ...
      <div className="flex flex-1 overflow-hidden">
        <div className="flex-1 relative">
          <MapContainer center={defaultCenter} zoom={withFix.length > 0 ? 12 : 6}
            ...
            {/* Draw ALL tracks (faded) when showAllTracks is on */}
            {showAllTracks && Object.entries(gpsHistory).map(([issi, track]) => {
              ...
              return (
                <Polyline key={`track-all-${issi}`}
                  ...
                  pathOptions={{ color, weight: isSelected ? 3 : 1.5, opacity: isSelected ? 0.9 : 0.35, dashArray: undefined }} />
              );
            })}

            {/* Draw selected track (always, opaque) */}
        ...
        {/* Station list / track history sidebar */}
        <div className="w-72 flex-shrink-0 border-l border-border bg-card overflow-y-auto flex flex-col">
          ...
          {positions.length === 0 ? (
            ...
            <div className="flex-1 overflow-y-auto">
                      ...
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {pos.hasFix ? (
                          <div className="text-right flex-shrink-0">
                            <div className="text-[10px] font-mono" style={{ color: trackColor }}>
                              {pos.lat.toFixed(4)}°
                            </div>
                            <div className="text-[10px] font-mono" style={{ color: trackColor }}>
                              {pos.lon.toFixed(4)}°
              ...
              {/* Track history for selected ISSI */}
              {selectedIssi && selectedTrack.length > 0 && (
