...
- **Neighbor Cells section** (`[[cell_info.neighbor_cells_ca]]`): toggle + count selector + dynamic per-cell forms (cell_identifier_ca, cell_reselection_types_supported, neighbor_cell_synchronized, cell_load_ca, main_carrier_number, mcc, mnc, location_area). When disabled, all blocks are written as `# [[cell_info.neighbor_cells_ca]]` with each field commented. cell_identifier_ca and location_area must be unique per cell. Server preserves & toggles all blocks on apply (`neighborCellsConfig` in payload), inserting them at the end of the `[cell_info]` direct-fields region (before any next section, including other `[[..]]` sub-tables).
